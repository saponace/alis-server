...
# Sonarr: automated TV shows download
    ...
    process_docker_compose_service sonarr "$(declare -p sonarr_docker_compose_template_mappings)"

# Radarr: automated Movies download
    declare -A  radarr_docker_compose_template_mappings=(
        ["RADARR_PORT"]="9004"
    )
    process_docker_compose_service radarr "$(declare -p radarr_docker_compose_template_mappings)"
...
# Jackett: tracker proxy for sonarr and radarr
    ...
    process_docker_compose_service jackett "$(declare -p jackett_docker_compose_template_mappings)"
#     ${INSTALL} jackett
